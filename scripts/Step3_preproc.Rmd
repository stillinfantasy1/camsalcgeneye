...
# subjects <- c('L003', 'L004', 'L006', 'L007', 'L009', 'L020', 'L021', 'H001', 'L022', 'L024', 'L026', 'L028', 'L029')
excl_subjects <- c('L025', 'L027', 'L023')

# Load anticipation data
append_gen_gaze_path <- file.path(preproc_path, "gaze", "append_gen_gaze.RData")
append_gen_gaze <- list()
if (file.exists(append_gen_gaze_path)) {
  load(append_gen_gaze_path)
}
...
for (subject in subjects) {
  ...
  append_gen_gaze[[subject]] <- ant
}
append_gen_gaze <- bind_rows(append_gen_gaze)
save(append_gen_gaze, file = append_gen_gaze_path)
```
...
append_gen_fix <- bind_rows(append_gen_fix)
...
Sdata <- make_pupillometryr_data(data = append_gen_gaze, subject = subject, trial = trial, time = time, condition = trial_cat)
...
# Add expect location info to gaze dataframe
gen_gaze_w_loc <- fix_corr %>%
...
save(gen_gaze_w_loc, file = preprocessed_gen_gaze_path)
```
