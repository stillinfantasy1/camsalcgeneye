...
proj_dir <- '~/Library/CloudStorage/Box-Box/CAMSLab_Projects/Studies/AlcGen/Data/In_Lab/'
...
preprocess_eye <- function(subject, task, phase, phase_data, baseline_window, out_dir, log_dir) {
  ...
  output_path <- paste0(out_dir, subject, '_', task, '_', phase, '_prep.csv') 
...
# read in data
```{r}
data <- read.delim(paste0(eye_dir, fname))
...
for (e in unique(timing_behav_merge$event_num)) {
  ...
  if (e == 1) {
    fixation_data = trial_eye
  } else {
    fixation_data <- rbind(fixation_data, trial_eye)
  }
}
...
```{r}
# leave fixation and anticipation alone
subject <- 'L003'
task <- 'conditioning'
prep_fix <- create_time_series(data = fixation_data, Adjust = 0) 
write_csv(prep_fix, paste0(out_dir, subject, '_', task, '_', 'fixation', '_prep.csv'))

prep_anticip <- create_time_series(data = anticip_data, Adjust = 0) 
write_csv(prep_anticip, paste0(out_dir, subject, '_', task, '_', 'anticipation', '_prep.csv'))
```
