...
append_gen_ant_df <- bind_rows(append_gen_ant, .id = "subject")
...
gen_twd <- gen_gaze_w_loc %>%
  ...
  #                                     ifelse(us_loc=="right", "left",
  #                                            ifelse(us_loc=="left", "right", NA))))) %>%
  mutate(xp_norm = (xp - fix_median) / fix_median, # positive = right, negative = left
         towardness = ifelse(expect_loc=="left", -1 * xp_norm, 
                               ifelse(expect_loc=="right", xp_norm,
                                           ifelse(expect_loc == "none", NA, NA)))) %>%
  ungroup()
...
P_gen_towardness <- ggplot(gen_twd %>% filter(stim %in% c("alc", "neut")), aes(x = time, y = towardness, color = stim)) + geom_smooth(se = FALSE) + labs(y="Towardness", x="Time") + facet_grid(~ gs_deg) + scale_color_manual(values = c("alc" = "#A06593", "neut" = "#6086C5")) + theme_classic() + geom_hline(yintercept = 0)

P_gen_sidebias <- ggplot(gen_twd %>% filter(stim %in% c("null", "novel")), aes(x = time, y = xp_norm, color = stim)) + geom_smooth(se = FALSE) + labs(y="Towardness", x="Time") + scale_color_manual(values = c("null" = "grey", "novel" = "black")) + theme_classic() + geom_hline(yintercept = 0)
